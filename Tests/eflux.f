	program  eflux
c Written by C. Constable
c Rewriting this June 2009 at GFZ- started form fflux7.f, stripping out forward and inverse parts
...
c Option exists to read predefined tesselation
c
c Evaluates flux integrals through patches on core surrounded by null flux curves
c Also will calculate flux (sined and unsigend?) through predefined regions on core (and perhaps Earth?) 
c surface to look at hemispheric asymmetries, temporal evolution, etc.
c
c see readme for what is needed for conversion of sh models to input here
c
c
...
c       ???????llpt(maxp,maxe),npatch,ipp(nxyz)??????
c
c	in /patchg/
...
c	
	...
	if=nint(val(1))
c	if (if.eq.1.or.if.eq.0)then
c
c   Read in core model with patch assignations
...
21	continue
c
c  For each epoch model, when ip=1, find patches on the core where Br has one sign
c
...
c  Compute flux through designated area, and axial dipole contribution
c
c  Cycle core points into p(4,j)
c
	   do 25 k=1,n
...
c
	parameter(nxyz=6500,nxyz2=2*nxyz,nxyz6=6*nxyz,nvert=10)
	parameter (maxob=7000,maxp=30,maxe=200)
	common /io/inp,iout,iprint
	common /tessel/n,ntria,p(4,nxyz),iv(3,nxyz2),near(nvert,nxyz)
	common /observ/ro(3,maxob,maxe),bro(maxob,maxe),sigma(maxob,
...
c--------------------------------------------------------------
	...
	implicit double precision (a-h,o-z)
c
c  Computes the flux through the designated regions  on the core,
...
c 10   gradm(j,k,i)=0.
c
c  Loop in patches, then in triangles within each patch.
      do 100 kkk=1,np(i)
c Flux in patch kkk at epoch i
	fluxv(kkk,i)=0.d0
      do 100 j=ltris(itris(kkk)),ltris(itris(kkk)+ntris(kkk)-1)
c  
c
c  compute centroid vc of current triangle
...
c
c  Find field value and sign of patch.
	    gam1=bint(rp,rg,k,p(1,iv(1,j)),p(1,iv(2,j)),p(1,iv(3,j)))
     ...
     $    ipp(iv(1,j)),ipp(iv(2,j)),ipp(iv(3,j)))
	  if(yes.le.0.and.ksignm(isign(kkk,i)).lt.0)then
		yes=1.d0
	  else if(yes.gt.0.And.ksignm(isign(kkk,i)).eq.1)then
		yes=1.d0
	  else
		yes=0.0
	  endif
...
c*******
            fluxv(kkk,i)=fluxv(kkk,i)+
     $        ajac*bjac*xyw(3,k)*gam1*yes
c	write(*,*) j, iv(1,j), ksignm(iv(1,j)),ndx, fluxv(ndx,i),i,br
c            do 40 l=1,3
c               if(signm(iv(l,j)).eq.sgnbr) 
c              gradm(lptr(j,ndx),iv(l,j),i)=gradm(lptr(j,ndx),
c     $         iv(l,j),i)+g(l)*ajac*bjac*xyw(3,k)
c 40         continue
c
c  Next integration sample point.
 50      continue
...
c  Add  next  to  lpts.
 ...
 60   continue
	do 61 j=1,np(i)
     ...
     $     ' sign ',isign(j,i)
	 write(iout,*)' points are ',(lpts(ipts(j,i)+k-1,i),k=1,npts(j,i))
61	continue
c
...
c----------------------------------------------------------
	...
	implicit double precision (a-h,o-z)
c
c  returns the sign of ipp at the ith vertex.
c* Calls no other routines.
c
      parameter (nxyz=6500, nvert=10, nxyz6=6*nxyz, nxyz2=2*nxyz)
	parameter(maxp=30,maxe=200)
...
c_______________________________________________________________________
	...
	implicit double precision (a-h,o-z)
c  interpolates patch identity yes or no to a point rg on the gnomonic projection
	common /rule/xyw(3,100),nquad
	dimension rp(3),rg(2),gamma(3),p1(4),p2(4),p3(4)
	gamma(1)=1.-xyw(1,k)-xyw(2,k)
	gamma(2)=xyw(1,k)
...
c	if(nfound.lt.0)nflag=1
	call getchr('tessel',filnam,nfound)
	if(nfound.lt.0)nflag=1
	call getchr('pointer',filnam,nfound)
	if(nfound.lt.0)nflag=1
c	call getval('obsdim',val,1,nfound)
c	if(nfound.lt.0)nflag=1
	call getval('bodydim',val,1,nfound)
	if(nfound.lt.0)nflag=1
c	call getval('design',val,1,nfound)
c	if(nfound.lt.0)nflag=1
	call getval('problem',val,1,nfound)
	if(nfound.lt.0)nflag=1
	call getval('epochs',val,1,nfound)
	if(nfound.lt.0)nflag=1
c	call getval('rmse',val,1,nfound)
c	if(nfound.eq.0)nflag=1
c	call getval('lambda',val,1,nfound)
c	if(nfound.eq.0)nflag=1
c	call getchr('e1format',filnam,nfound)
c	if(nfound.lt.0)nflag=1
c
...
c  
	parameter(nxyz=6500,nxyz2=2*nxyz,nxyz6=6*nxyz,nvert=10)
	parameter (maxp=30,lmax=101,maxe=200)
	common /io/inp,iout,iprint
	common /tessel/n,ntria,p(4,nxyz),iv(3,nxyz2),near(nvert,nxyz)
        common/patchy/np(maxe),npts(maxp,maxe),isign(maxp,maxe),
     $   ipts(maxp,maxe),
