      program fieldpred3k
      implicit none
c     A program to give model prediction time series of declination, 
c     inclination and intensity at the Earth's surface for an input location 
c     from models ARCH3k_cst.1 or CALS3k_cst.1
c     These three model files have to be in the same directory as the program.
c     Monika Korte, October 2008
c
...
c--------------------------------------------------------------------
      ...
      integer it1,it2,lm,nm,k,j,i,nleft,modflag
      ...
      write(*,*) 'as given by models CALS3k_cst.1 or ARCH3k_cst.1.'
      write(*,*) 'Results are written to a plain text output file.'
      write(*,*) 
      write(*,*) 'Choose model 1 or 2:'
      write(*,*) '1 - CALSk_cst.1'
      write(*,*) '2 - ARCH3k_cst.1'
      read(*,*) modflag
...
c     read model
      if (modflag.eq.1) then
         modfile='CALS3k_cst.1'
      else if (modflag.eq.2) then
         modfile='ARCH3k_cst.1'
      else
      ...
      write(11,*) 'time d i f lat=',alat,' long=',alon
      write(11,*) 'Predictions from ',modfile 
...
c     convert inclination and declination to degrees
      ...
      write(11,6200) i,d,ainc,f/1000.
      end do
99    continue
6200  format(i6,2f8.2,f6.1)
      close(11)
c********************************************************************
      stop
      end
